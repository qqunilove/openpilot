...
def disable_radar(logcan, sendcan, bus=1, timeout=0.1, debug=False):
  ...
  Openpilot will emulate the radar. WARNING: THIS DISABLES AEB!"""
    ...
    try:
        query = IsoTpParallelQuery(sendcan, logcan, bus, [RADAR_ADDR], [EXT_DIAG_REQUEST], [EXT_DIAG_RESPONSE],
                                   debug=debug)

        for _, _ in query.get_data(timeout).items():
            cloudlog.warning("radar communication control disable tx/rx ...")

            query = IsoTpParallelQuery(sendcan, logcan, bus, [RADAR_ADDR], [COM_CONT_REQUEST], [COM_CONT_RESPONSE],
                                       debug=debug)
            query.get_data(0)
...
def create_brake_command(packer, apply_brake, pump_on, pcm_override, pcm_cancel_cmd, fcw, idx, car_fingerprint,
                         stock_brake):
...
def create_ui_commands(packer, pcm_speed, hud, car_fingerprint, is_metric, idx, openpilot_longitudinal_control,
                       stock_hud):
    commands = []
